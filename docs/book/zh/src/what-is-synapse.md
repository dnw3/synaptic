...
Synapse 是一个用于构建 AI Agent、链式调用和检索管道的 Rust 框架。它遵循与 [LangChain](https://python.langchain.com/)（Python）相同的架构和抽象，并将其转化为地道的 Rust 风格：强类型、异步原生和零开销抽象。

如果你使用过 LangChain Python，那么你已经熟悉了 Synapse 的思维模型。Synapse 提供相同的可组合构建块 -- Chat Models、Prompts、Output Parsers、Runnables、Tools、Memory、Graph 和 Retrieval -- 但在编译时而非运行时捕获错误。

## LangChain 到 Synapse 的映射

下表展示了 LangChain Python 的核心概念如何映射到 Synapse Rust 的对应实现：

| LangChain (Python) | Synapse (Rust) | 所在 Crate |
|---|---|---|
...
| `RunnableSequence` / LCEL `\|` | `BoxRunnable` / `\|` 管道运算符 | `synapse-runnables` |
...
虽然架构兼容，但 Synapse 做出了符合 Rust 惯例的设计选择：
...
- **错误是有类型的**。`SynapseError` 是一个包含 19 个变体的枚举（每个子系统一个），而非通用异常。
...
- **性能关键的 AI 应用** -- Rust 的零开销抽象和无垃圾回收机制使 Synapse 适合高吞吐、低延迟的 Agent 工作负载。没有 Python GIL 限制并发。
- **Rust 生态系统集成** -- 如果你的应用已经用 Rust 编写（Axum/Actix Web 服务、CLI 工具、嵌入式系统），Synapse 让你无需跨越 FFI 边界或管理 Python 子进程即可添加 AI Agent 能力。
- **编译时安全** -- 工具参数 schema、消息类型和 Runnable 管道签名都由编译器检查。重构工具的输入类型时，编译器会在每个调用点报错，而不是在生产环境中运行时崩溃。
- **可部署的二进制文件** -- Synapse 编译为单个静态二进制文件，无运行时依赖。不需要 Python 解释器、虚拟环境或 pip install。
