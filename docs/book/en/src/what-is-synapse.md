...
Synapse is a Rust framework for building AI agents, chains, and retrieval pipelines. It follows the same architecture and abstractions as [LangChain](https://python.langchain.com/) (Python), translated into idiomatic Rust with strong typing, async-native design, and zero-cost abstractions.

If you have used LangChain in Python, you already know the mental model. Synapse provides the same composable building blocks -- chat models, prompts, output parsers, runnables, tools, memory, graphs, and retrieval -- but catches errors at compile time instead of runtime.

## LangChain to Synapse Mapping

The table below shows how core LangChain Python concepts map to their Synapse Rust equivalents:
...
While the architecture is compatible, Synapse makes deliberate Rust-idiomatic choices:
...
- **Errors are typed**. `SynapseError` is an enum with 19 variants (one per subsystem), not a generic exception.
...
Synapse is a good fit when you need:

- **Performance-critical AI applications** -- Rust's zero-cost abstractions and lack of garbage collection make Synapse suitable for high-throughput, low-latency agent workloads. There is no Python GIL limiting concurrency.
- **Rust ecosystem integration** -- If your application is already written in Rust (web servers with Axum/Actix, CLI tools, embedded systems), Synapse lets you add AI agent capabilities without crossing an FFI boundary or managing a Python subprocess.
...
- **Deployable binaries** -- Synapse compiles to a single static binary with no runtime dependencies. No Python interpreter, no virtual environment, no pip install.
