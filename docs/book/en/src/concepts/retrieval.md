...
pub trait TextSplitter: Send + Sync {
    fn split_text(&self, text: &str) -> Result<Vec<String>, SynapseError>;
    fn split_documents(&self, documents: Vec<Document>) -> Result<Vec<Document>, SynapseError>;
}
```
...
pub trait Embeddings: Send + Sync {
    async fn embed_documents(&self, texts: Vec<String>) -> Result<Vec<Vec<f32>>, SynapseError>;
...
pub trait VectorStore: Send + Sync {
    async fn add_documents(&self, docs: Vec<Document>, embeddings: Vec<Vec<f32>>) -> Result<Vec<String>, SynapseError>;
    async fn similarity_search(&self, query_embedding: &[f32], k: usize) -> Result<Vec<Document>, SynapseError>;
    async fn delete(&self, ids: &[String]) -> Result<(), SynapseError>;
}
```
...
pub trait Retriever: Send + Sync {
    async fn retrieve(&self, query: &str) -> Result<Vec<Document>, SynapseError>;
}
```

A retriever takes a natural-language query and returns relevant documents. Synapse provides seven retriever implementations, each with different strengths.
...
let docs = retriever.retrieve("What is Synapse?").await?;
