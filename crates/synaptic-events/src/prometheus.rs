...
//!
//! Wraps [`MetricsCallback`] from the `observers` module and exposes collected
...
//! ```no_run
//! use std::sync::Arc;
//! use synaptic_events::observers::MetricsCallback;
//! use synaptic_events::prometheus::PrometheusExporter;
//!
//! # async fn example() {
...
use crate::observers::{MetricsCallback, MetricsSnapshot, ToolMetrics};
...
mod tests {
    ...
    async fn exporter_render() {
        use crate::{Event, EventKind, EventSubscriber};

        let cb = Arc::new(MetricsCallback::new());

        // Simulate a model call.
        let mut ev_input = Event::new(EventKind::LlmInput, serde_json::json!({"message_count": 1}))
            .with_request_id("r1");
        cb.handle(&mut ev_input).await.unwrap();
        let mut ev_output = Event::new(
            EventKind::LlmOutput,
