...
///
/// ```rust,no_run
/// use synaptic_store::mongodb::MongoCheckpointer;
///
...
impl MongoCheckpointer {
    ...
    pub async fn new(db: &Database, collection_name: &str) -> Result<Self, SynapticError> {
        ...
        // Index on (thread_id, created_at) for ordered listing and latest retrieval
...
impl Checkpointer for MongoCheckpointer {
    async fn put(
        ...
        // Use a timestamp-based ordering key to avoid the count-then-insert race.
        // BsonDateTime::now() provides monotonic ordering within a single process,
        // and unique checkpoint_id ensures idempotent upserts.
        let document = doc! {
