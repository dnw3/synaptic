...
//! Apple containers. Requires the `sandbox-apple` feature flag and must be
//! compiled for macOS (`target_os = "macos"`).

#[cfg(all(target_os = "macos", feature = "sandbox-apple"))]
mod inner {
    ...
    use crate::sandbox::{ResourceLimits, SandboxBackend, SandboxResult};
