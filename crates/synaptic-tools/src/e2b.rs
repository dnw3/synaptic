...
//!
//! Requires the `e2b` feature flag.
//!
//! # Example
//!
//! ```rust,ignore
//! use synaptic_tools::e2b::{E2BConfig, E2BSandboxTool};
...
impl E2BSandboxTool {
    ...
    pub fn new(config: E2BConfig) -> Self {
        let client = reqwest::Client::builder()
            .timeout(std::time::Duration::from_secs(config.timeout_secs + 10))
            .build()
            .unwrap_or_default();
        Self { config, client }
    }
...
impl Tool for E2BSandboxTool {
    ...
    async fn call(&self, args: Value) -> Result<Value, SynapticError> {
        ...
        // Validate language to prevent unexpected fallthrough to bash
        const ALLOWED_LANGUAGES: &[&str] = &["python", "javascript", "js", "bash"];
        if !ALLOWED_LANGUAGES.contains(&language) {
            return Err(SynapticError::Tool(format!(
                "Unsupported language '{language}'. Allowed: python, javascript, bash"
            )));
        }
