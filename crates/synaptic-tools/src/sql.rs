...
//! use synaptic_tools::sql::SqlToolkit;
...
impl Tool for ExecuteQueryTool {
    ...
    async fn call(&self, args: Value) -> Result<Value, SynapticError> {
        ...
        // Reject multi-statement queries (semicolons within the query body).
        ...
        // Strip trailing semicolons/whitespace, then reject if any semicolons remain
        // (indicates multi-statement attempt like "SELECT 1; DROP TABLE ...").
        let body = query.trim().trim_end_matches(';').trim();
        if body.contains(';') {
            return Err(SynapticError::Tool(
                "Multi-statement queries are not allowed for safety.".to_string(),
            ));
        }
