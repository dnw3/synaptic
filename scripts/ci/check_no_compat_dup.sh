#!/usr/bin/env bash
set -euo pipefail
# Prevent OpenAI wrapper pattern from proliferating outside synaptic-models::openai.
# Scans all crate src/ directories for the combination of:
#   1. importing synaptic_models::openai
#   2. implementing ChatModel for a local type
# Allowlist: crates that genuinely implement their own ChatModel.
ALLOW="synaptic-models|synaptic-anthropic|synaptic-gemini|synaptic-ollama|synaptic-bedrock"
FAIL=0
for crate_dir in crates/synaptic-*/; do
  ...
  [[ -d "$src_dir" ]] || continue
  if rg -q 'use synaptic_models::openai' "$src_dir" 2>/dev/null && \
